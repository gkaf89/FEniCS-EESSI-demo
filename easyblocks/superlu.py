...
class EB_SuperLU(CMakeMake):
    ...
    def extra_options():
        ...
        extra_vars.update({
            'mpiexec': ['mpiexec', "Provide the scheduler launch command if your are building withing a job; "
                        "it helps resolve some oversubscription issues", CUSTOM],
    ...
    def _setup_mpiexec(self):
        mpiexec = self.cfg.get('mpiexec')
        if mpiexec:
            mpiexec_cmd = mpiexec + ' ' + self.cfg['mpiexec_numproc_flag'] + ' 1'
            if self.cfg.get('pretestopts'):
                self.cfg['pretestopts'] += mpiexec_cmd + ' '
            else:
                self.cfg.update('pretestopts', mpiexec_cmd + ' ')
